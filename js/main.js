...
import {copyText, focusSoon, getValue, keepAwake, plural, setValue, toast} from './ui.js';
...
const EXAMPLES = ['Hund', 'Katze', 'Schule', 'Baum', 'fahren', 'spielen', 'Haus', 'Mutter', 'Vater', 'groß', 'klein', 'Fisch'];
...
function render(){
  ...
  document.getElementById('count').textContent = n ? plural(n, 'Wort', 'Wörter') : '';
  document.getElementById('tabTrain').setAttribute('aria-selected', state.tab === 'train');
  document.getElementById('tabWords').setAttribute('aria-selected', state.tab === 'words');
...
function commit(words = state.words){
  state.words = words;
  if (!store.saveWords(words)) toast('Speichern hat nicht geklappt. Ist privates Surfen aktiv?');
  render();
}
...
function addWords(texts, list){
  ...
  if (fresh.length){
    ...
    toast(fresh.length === 1 ? `„${fresh[0]}“ hinzugefügt` : `${fresh.length} Wörter hinzugefügt`);
...
const actions = {
  ...
  examples(){ addWords(EXAMPLES, 'Beispiel'); },

  // Üben
  start(){
    const ids = pickRound(wordsInList(state.words, state.filter), state.size);
    if (!ids.length){ toast('In dieser Auswahl sind keine Wörter.'); return; }
  ...
  reveal(){ state.round.revealed = true; render(); },
  toggleHide(){ state.hide = !state.hide; savePrefs(); render(); },
  skip(){ state.round.idx++; state.round.revealed = false; render(); },
  mark(el){
    const round = state.round, id = round.ids[round.idx], res = el.dataset.res;
    round.results[id] = res;
    round.idx++;
    round.revealed = false;
    if (round.idx >= round.ids.length) keepAwake(false);
    const w = state.words.find(x => x.id === id);
    if (w) updateWord(id, {right: (w.right || 0) + (res === 'good' ? 1 : 0), wrong: (w.wrong || 0) + (res === 'warn' ? 1 : 0), lastAt: Date.now()});
    else render();
  },
  ...
  saveEdit(){
    const w = currentWord();
    if (!w) return;
    const text = clean(getValue('editText')), list = clean(getValue('editList'));
    if (!text){ toast('Das Wort darf nicht leer sein.'); return; }
    if (text !== w.text && state.words.some(x => x.text === text)){ toast('Dieses Wort gibt es schon.'); return; }
    closeModal();
  ...
  askConfirm(){ state.confirm = true; renderModal(modalRoot, state); },
  cancelConfirm(){ state.confirm = false; renderModal(modalRoot, state); },
  closeModal,

  // Teilen
  openShare(){ openModal({type: 'share'}); },
  async doShare(){
    const text = buildMessage(sharedWords(), appUrl());
    if (navigator.share){
      try { await navigator.share({text}); closeModal(); }
      catch (err){
        if (err?.name !== 'AbortError' && await copyText(text)) toast('Teilen ging nicht. Text ist kopiert.');
      }
  ...
  async copyIncoming(){
    toast(await copyText(state.modal.text) ? 'Kopiert. Jetzt die App öffnen.' : 'Kopieren hat nicht geklappt.');
  },
  doImportMerge(){ importWords('merge'); },
  doImportReplace(){ importWords('replace'); },
};

function importWords(mode){
  const {words, count} = applyImport(state.words, parseMessage(state.modal.text), mode);
  closeModal();
  state.tab = 'words';
  commit(words);
  if (mode === 'replace') toast(`Liste ersetzt: ${plural(count, 'Wort', 'Wörter')}`);
  else toast(count ? `${count} neue ${count === 1 ? 'Wort' : 'Wörter'} übernommen` : 'Alle Wörter waren schon da');
}
...
document.addEventListener('click', e => {
  ...
  if (filter){ state.filter = filter.dataset.filter; savePrefs(); render(); return; }

  const size = t.closest('[data-size]');
  if (size){ state.size = Number(size.dataset.size); savePrefs(); render(); return; }

  const shareList = t.closest('[data-sharelist]');
  if (shareList){ state.shareList = shareList.dataset.sharelist; renderModal(modalRoot, state); return; }

  const edit = t.closest('[data-edit]');
  if (edit){ openModal({type: 'edit', id: edit.dataset.edit}); return; }
...
document.addEventListener('input', e => {
  if (e.target.id === 'search'){
    ...
    searchTimer = setTimeout(() => { state.search = e.target.value; render(); }, 120);
  }
  if (e.target.id === 'importText' && state.modal){
    state.modal.text = e.target.value;
    document.getElementById('importPreview').innerHTML = importPreview(state.modal.text, state.words);
    document.getElementById('importActions').hidden = !parseMessage(state.modal.text).length;
  }
});
...
document.addEventListener('visibilitychange', () => {
  const r = state.round;
  if (document.visibilityState === 'visible' && r && r.idx < r.ids.length) keepAwake(true);
});
...
window.matchMedia('(prefers-color-scheme: dark)').addEventListener?.('change', redrawCard);
document.fonts?.load('400 40px "Andika"').then(redrawCard).catch(() => {});
...
function checkIncomingLink(){
  ...
  if (parseMessage(text).length) openModal({type: 'import', text, fromLink: true});
}
