...
export const newId = () => 'w' + Date.now().toString(36) + Math.random().toString(36).slice(2, 7);

export function createWord(text, list = '', now = Date.now()){
  return {id: newId(), text: clean(text), list: clean(list), right: 0, wrong: 0, lastAt: 0, createdAt: now};
}
...
export function listNames(words){
  return [...new Set(words.map(w => (w.list || '').trim()).filter(Boolean))].sort(byName);
}
