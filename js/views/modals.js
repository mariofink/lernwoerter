...
function renderEdit(root, state, m) {
  ...
  const typedText = getValue("editText"),
    typedList = getValue("editList");
  const last = w.lastAt
    ? ", zuletzt am " +
      new Date(w.lastAt).toLocaleDateString("de-DE", {
        day: "numeric",
        month: "long",
      })
    : "";
  root.innerHTML = shell(
    ...
    <div class="field"><label for="editList">Liste</label>
      ...
      <datalist id="listOptionsEdit">${listNames(state.words)
...
function renderShare(root, state) {
  ...
  if (state.shareList !== "alle" && !lists.includes(state.shareList))
    state.shareList = "alle";
  ...
  root.innerHTML = shell(
    ...
    ${
      lists.length
        ? `<div class="stack" style="gap:8px"><p class="eyebrow" style="margin:0">Was teilen?</p><div class="row">
...
function renderImport(root, state, m) {
  ...
  root.innerHTML = shell(
    ...
    ${
      openedInSafari
        ? `<p class="notice">Nutzt du die Lernwörter-Kiste auf dem Home-Bildschirm? Dann tippe auf <b>Nachricht kopieren</b>, öffne die App und füge sie dort unter „Liste empfangen“ ein. Die App auf dem Home-Bildschirm hat einen eigenen Speicher.</p>
    ...
    ${
      m.fromLink
        ? ""
        : `<p class="muted small" style="margin:0">Kopiere die ganze Nachricht mit der Wortliste und füge sie hier ein.</p>
...
export function importPreview(text, words) {
  ...
  if (!stats.total)
    return '<div class="preview bad">In diesem Text wurden keine Lernwörter gefunden. Kopiere bitte die ganze Nachricht.</div>';
