...
export function renderTrain(root, state) {
  ...
  if (!round) {
    root.innerHTML = startScreen(state);
    return;
  }
  if (round.idx >= round.ids.length) {
    root.innerHTML = summary(state);
    return;
  }

  const word = state.words.find((w) => w.id === round.ids[round.idx]);
  if (!word) {
    round.ids.splice(round.idx, 1);
    renderTrain(root, state);
    return;
  }
...
function startScreen(state) {
  ...
  if (state.filter !== "alle" && !lists.includes(state.filter))
    state.filter = "alle";
  ...
  return `<div class="stack">
    <section class="panel stack">
      ...
      ${
        lists.length
          ? `<div class="stack" style="gap:8px"><p class="eyebrow" style="margin:0">Welche Wörter?</p>
        <div class="row">
...
function card(state, word) {
  ...
  const progress = round.ids
    .map((id, i) => {
      const cls = round.results[id]
        ? "done-" + round.results[id]
        : i === round.idx
          ? "now"
          : "";
...
function summary(state) {
  ...
  const items = done
    .map((id) => {
      ...
      if (!w) return "";
      const pill =
        round.results[id] === "good"
          ? '<span class="pill good">richtig</span>'
          : '<span class="pill warn">noch üben</span>';
