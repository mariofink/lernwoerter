...
test('Hin und zurück: Umlaute, ß und Listen bleiben erhalten', () => {
  const words = [word('Straße', 'Woche 5'), word('Übung', 'Woche 5'), word('Mäuse'), word('groß', 'Diktat Tiere')];
  const items = parseMessage(buildMessage(words, APP));
  assert.deepEqual(sortItems(items), sortItems(words.map(w => ({text: w.text, list: w.list}))));
});
...
test('groupWords: Listen sortiert, „Ohne Liste“ zuletzt, Woche 2 vor Woche 10', () => {
  const groups = groupWords([word('a', 'Woche 10'), word('b'), word('c', 'Woche 2')]);
  assert.deepEqual(groups.map(g => g[0]), ['Woche 2', 'Woche 10', '']);
});

test('importStats zählt neue und doppelte Wörter richtig', () => {
  const items = [{text: 'Hund', list: ''}, {text: 'Katze', list: ''}, {text: 'Katze', list: 'X'}];
...
test('Hinzufügen: nur neue Wörter, vorhandene bleiben unverändert', () => {
  ...
  const {words, count} = applyImport([hund], [{text: 'Hund', list: 'Neu'}, {text: 'Katze', list: 'Neu'}, {text: 'Katze', list: 'Neu'}], 'merge');
...
test('Ersetzen: Zähler bleiben bei gemeinsamen Wörtern, Rest fällt weg', () => {
  ...
  const {words, count} = applyImport(old, [{text: 'Hund', list: 'Neu'}, {text: 'Katze', list: ''}], 'replace');
  assert.equal(count, 2);
  assert.deepEqual(words.map(w => w.text), ['Hund', 'Katze']);
