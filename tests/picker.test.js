...
const word = (id, extra = {}) => ({id, text: id, list: '', right: 0, wrong: 0, lastAt: 0, ...extra});
...
test('Gibt es weniger Wörter als gewünscht, kommen alle genau einmal', () => {
  const ids = pickRound([word('a'), word('b')], 5, {random: seeded(1), now: NOW});
...
test('Wackelige Wörter kommen in vielen Runden deutlich öfter dran', () => {
  const words = [word('wackelig', {right: 0, wrong: 4}), ...Array.from({length: 9}, (_, i) => word('sicher' + i, {right: 5}))];
  const random = seeded(42);
  let hits = 0;
  for (let i = 0; i < 2000; i++) if (pickRound(words, 1, {random, now: NOW})[0] === 'wackelig') hits++;
